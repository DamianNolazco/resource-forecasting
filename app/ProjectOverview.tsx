...
export function ProjectOverview({plan,onPerson,onAllocation,onMilestone,onProject}:{plan:Plan;onPerson:(p:Person)=>void;onAllocation:(b:PlanBar)=>void;onMilestone:(m:Milestone)=>void;onProject:(id:string)=>void}){
 ...
 return <section className="panel overview-panel" aria-label="Project allocation overview">
  ...
  <div className="overview-key"><span>Lines show assignments · Hover for dates and allocation · Red dots indicate overallocation.</span><span>◆ Internal milestone · ◇ External milestone</span></div>
      ...
      {!folded&&<>
       ...
       {lanes.map(lane=>{
        ...
        return <div className="overview-row" key={lane.key} data-department={lane.dept}>
         ...
         <div className="overview-track" style={{height}}>{current>=start&&current<=end&&<span className="overview-today" style={{left:`${position(current+.5)}%`}}/>}{stacked.map(b=>{const conflict=!!b.personId&&months.some(m=>m.index>=b.start&&m.index<=b.end&&allocated(plan.bars,b.personId!,m.index)>1);return <button key={b.id} className={`overview-bar avatar-${b.dept} ${!b.personId?'unassigned':''} ${conflict?'conflict':''}`} style={{left:`${position(Math.max(start,b.start))}%`,width:`${(Math.min(end,b.end)-Math.max(start,b.start)+1)/count*100}%`,top:9+b.track*35}} onClick={()=>onAllocation(b)} aria-label={`${project.name}, ${lane.person?.name||'Unassigned'}, ${b.dept}, ${pct(b.allocation)}, ${monthLabel(b.start)} to ${monthLabel(b.end)}${conflict?', overallocated across projects':''}`} title={`${lane.person?.name||'Unassigned'} · ${b.dept} · ${pct(b.allocation)}\n${monthLabel(b.start)} – ${monthLabel(b.end)}${conflict?'\nAbove 100% across all projects':''}`}><b>{lane.person?.name||'Unassigned'}</b>{conflict&&<span className="overview-conflict-dot" aria-hidden="true">●</span>}</button>})}</div>
        </div>;
       })}
